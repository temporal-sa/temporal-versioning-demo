...
- [Temporal API constraints](references/temporal-api-constraints.md) — no `.` in deployment name; no `ORDER BY` on dev visibility (sort in Go, keep newest tail); visibility omits VersioningInfo → filter by `TemporalWorkerDeploymentVersion` SA, not `pinnedBuildID` (caused Recover "0" + zeroed "N in flight" counts, both fixed; never key per-order aggregates on `LiveOrder.BuildID`); ramp/promote need AllowNoPollers + IgnoreMissingTaskQueues.
...
- [Deployment panel UI](references/deployment-panel-ui.md) — Deploy/Rollback row (SSE region `#controls`; Rollback disabled unless a version is Ramping); Recover is now per-card (amber ↻ `.recover-btn` on a failing order → `POST /api/recover/{id}` → `RecoverOne`, htmx self-disable, no server busy state), no KPI band, cards sorted v1→v2→v3 showing `N in flight`; no failure count (infinite retry → never Failed).
