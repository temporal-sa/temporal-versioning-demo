...
- [Worker versioning model](references/worker-versioning-model.md) — single image + PIZZA_VERSION = Pinned shape; Manual controller, UI-driven routing, backend auto-promotes only the first version; target `temporal-k8s` Kind cluster.
...
- [Demo timing & v3 drone regression](references/demo-timing.md) — StepDwell 15 s, DeliveredDwell 7 s (sized to outlast UI collapse); order every ~6 s; ramp 10/25/50/100 %; v3 drone always fails into a bounded retry loop.
