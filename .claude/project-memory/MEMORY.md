...
- [Deployment panel UI](references/deployment-panel-ui.md) — Deploy/Rollback/Recover row (now an SSE region `#controls`; Rollback disabled unless a version is Ramping, Recover disabled unless an order is failing — `hasFailing(.Orders)`), no KPI band, cards sorted v1→v2→v3 showing `N in flight`; no failure count (infinite retry → never Failed).
