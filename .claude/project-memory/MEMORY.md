...
- [Temporal API constraints](references/temporal-api-constraints.md) — no `.` in deployment name; no `ORDER BY` on dev visibility (sort in Go, keep newest tail); visibility omits VersioningInfo → filter by `TemporalWorkerDeploymentVersion` SA, not `pinnedBuildID` (caused Recover "0"; also breaks "N in flight" counts); ramp/promote need AllowNoPollers + IgnoreMissingTaskQueues.
