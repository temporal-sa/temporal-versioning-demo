...
- [Kubernetes namespace: pizza-tracker](references/k8s-namespace.md) — demo deploys to a dedicated `pizza-tracker` namespace owned by Kustomize (not `default`); the `temporalNamespace`/`TEMPORAL_NAMESPACE`/`parentRefs` namespaces are unrelated and stay as-is.
