...
- [Frontend conventions & gotchas](references/frontend-conventions.md) — Tailwind compiled ahead-of-time (build step → `app.css`, served as static `<link>`), HTMX-only, hypermedia URLs off `/api/`; traps: `@media` can't `@apply`, `#orders`/`#versions` must morph not replace, close modals with empty 200.
