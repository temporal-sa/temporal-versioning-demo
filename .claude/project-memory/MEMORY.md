...
- [Kubernetes namespace: pizza-tracker](references/k8s-namespace.md) — demo deploys to a dedicated `pizza-tracker` namespace owned by Kustomize (not `default`); controller names the deployment `<ns>/<name>` so backend derives `PIZZA_DEPLOYMENT_NAME=$(POD_NAMESPACE)/pizza` via Downward API; the `temporalNamespace`/`TEMPORAL_NAMESPACE`/`parentRefs` namespaces are unrelated and stay as-is.
